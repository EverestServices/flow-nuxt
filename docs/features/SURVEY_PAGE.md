...
**Overview:**
Conditional info messages allow displaying contextual information, warnings, or danger alerts next to questions based on the current field values. These messages appear as colored icons with tooltips when specific conditions are met.

**Interface (`surveyInvestments.ts`):**
```typescript
export interface ConditionalInfoMessage {
  condition: {
    field: string        // Field name to check ('self' for the question itself)
...
// Added to SurveyQuestion interface:
conditional_info_messages?: ConditionalInfoMessage[]
```

**Display Component (`SurveyQuestionConditionalInfo.vue`):**
- Displays info/warning/danger icon with tooltip
- Uses `UPopover` for hover-based tooltip (no TooltipProvider context required)
- Icon colors: info (blue), warning (yellow), danger (red)
- Danger type shows red border on tooltip
- Supports multiline messages with `whitespace-pre-line`

**Evaluation Logic (`SurveyQuestionRenderer.vue`):**
```typescript
const activeConditionalMessage = computed(() => {
  // Checks each conditional_info_messages entry
  // Returns first message where condition is met
  // Supports 'self' field to check question's own value
  // Supports type coercion for boolean comparisons
})
```

**Usage Example:**
```typescript
// Show danger message when defect switch is enabled
{
  conditional_info_messages: [
    {
      condition: {
        field: 'self',
        operator: 'equals',
      ...
      message_translations: {
        hu: 'Hibajelzés aktív! Kérjük, dokumentálja a hibát.',
        en: 'Defect detected! Please document the issue.'
      }
    }
  ]
}
...
**Visual Behavior:**
- Icon appears immediately left of switch/toggle controls
- For other question types, icon appears next to the label
- Tooltip displays on hover
- Real-time evaluation as field values change

### Calculated Field Unit Conversions

**Overview:**
Calculated fields can now apply unit conversions to field values before performing calculations. This is essential for accurate area calculations where different fields use different units (e.g., cm vs. m).

**Configuration (`SurveyQuestionRenderer.vue`):**
```typescript
// Formula options for calculated fields
{
  operation: 'multiply' | 'add' | 'subtract' | 'divide',
  fields: string[],
  decimals?: number,
  field_unit_conversions?: Record<string, number>  // NEW: Multipliers for unit conversion
}
```
...
const calculatedValue = computed(() => {
  // 1. Get values for all fields in formula
  const values: number[] = []
  for (const fieldName of formula.fields) {
    let numValue = parseFloat(store.getResponse(fieldName))

    // 2. Apply unit conversion if specified
    if (formula.field_unit_conversions?.[fieldName]) {
  ...
  // 4. Format with specified decimals
})
```

**Usage Example (Migration 173):**
```sql
-- Foundation area calculation: wall_length (cm) × foundation_height (cm) → m²
...
SET options = jsonb_build_object(
  ...
  'field_unit_conversions', jsonb_build_object(
    'wall_length', 0.01,        -- Convert cm to m
    'foundation_height', 0.01   -- Convert cm to m
  )
)
WHERE type = 'calculated' AND name = 'foundation_area';
```

**Common Conversion Scenarios:**
- **cm → m:** Multiply by `0.01`
- **mm → m:** Multiply by `0.001`
- **m² → cm²:** Multiply by `10000`
- **Custom conversions:** Any numeric multiplier

**Benefits:**
- Accurate calculations across mixed units
- No need to manually convert in UI
- Database stores raw values in original units
- Display shows correctly converted results
- Automatic recalculation on value change

### Related Migrations

**172_add_foundation_type_to_basic_data.sql:**
- Adds `foundation_type` question to facade basic data page
- Links to Walls page `foundation_type` via `default_value_source_question_id`

**173_update_foundation_area_calculation.sql:**
- Updates `foundation_area` calculation with unit conversions
- Converts `foundation_height` from cm to m (× 0.01)
- Automatically finds and updates all calculated questions using `foundation_height`
