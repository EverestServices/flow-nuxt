...
// Get scenario components for a category
const getCategoryComponents = (categoryId: string) => {
  return scenariosStore.getScenarioComponentForCategory(categoryId)
}
...
const handleAddRow = async (categoryId: string) => {
  ...
  try {
    ...
    await scenariosStore.addScenarioComponent(availableComponent.value, 1)
  } catch (error) {
